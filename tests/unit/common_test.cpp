...
TEST(CommonTest, read_write_vector_serializable) {
	...
	SerializableHelper::write_vector_serializeable(oss, vector);

	std::istringstream iss(oss.str(), std::ios::binary);
	std::vector<FileListing> vector_back = SerializableHelper::read_vector_serializeable<FileListing>(iss);
